...
public class ReflectionInstances {
        ...
        for(Field selectedField: cls.getDeclaredFields()){
            ...
            try {
                ...
                if(selectedField.getType()==Integer.class || selectedField.getType()==int.class){
                    setter.invoke(selectedClazz,convertToInt(propertyValue));
                }else if(selectedField.getType()==Instant.class){
                    setter.invoke(selectedClazz,convertToInstant(propertyValue));
                }else if(selectedField.getType()==Date.class){
                    setter.invoke(selectedClazz,convertToDate(propertyValue, dataFormat));
                }else {
    ...
    private static Instant convertToInstant(String possibleInstant){
    ...
    private static Date convertToDate(String possibleData,String dateFormat) {
        ...
        try {
            ...
            throw new CustomParseException(String.format("Can't parse format %1$s",dateFormat));
        }
        return date;
    }
